...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Aged Receivables', href: '/accounting/reports/aged-receivables' },
];

export default function AgedReceivables({ rows }: { rows: AgedRow[] }) {
    return (
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Outstanding amounts owed by customers, grouped by age.</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {['csv', 'xlsx', 'pdf'].map((format) => (
                            <Button key={format} asChild variant="outline">
                                <Link href={`/accounting/reports/aged-receivables/export/${format}`}>
                    ...
                    <table className="w-full min-w-[900px] text-sm">
                        ...
                        <tbody>
                            {rows.length ? rows.map((row) => (
                                ...
                                <tr><td className="p-6 text-center text-muted-foreground" colSpan={7}>No outstanding receivables found.</td></tr>
                            )}
                        </tbody>
                        {rows.length > 0 && (
...
AgedReceivables.layout = { breadcrumbs };
