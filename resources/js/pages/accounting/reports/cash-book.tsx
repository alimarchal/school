...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Cash Book', href: '/accounting/reports/cash-book' },
];

export default function CashBook({ entries, totals, filters }: { entries: PaginatedEntries; totals: Totals; filters: Record<string, string> }) {
    const [dateFrom, setDateFrom] = useState(filters.date_from ?? '');
    const [dateTo, setDateTo] = useState(filters.date_to ?? '');

    function applyFilters() {
        router.get('/accounting/reports/cash-book', { date_from: dateFrom, date_to: dateTo }, { preserveState: true });
    }
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">All cash account transactions for the selected period.</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {['csv', 'xlsx', 'pdf'].map((format) => (
                            <Button key={format} asChild variant="outline">
                                <Link href={`/accounting/reports/cash-book/export/${format}?date_from=${dateFrom}&date_to=${dateTo}`}>
                    ...
                    <table className="w-full min-w-[800px] text-sm">
                        ...
                        <tbody>
                            {entries.data.length ? entries.data.map((row, i) => (
                                ...
                                <tr><td className="p-6 text-center text-muted-foreground" colSpan={6}>No cash transactions found for this period.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
